...
public class ObservationPostRequest {
     ...
     * @throws IOException if the JSON Element is invalid with respect to ObservationPostRequest
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        if (jsonElement == null) {
            if (!ObservationPostRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
                throw new IllegalArgumentException(String.format(
                        java.util.Locale.ROOT,
                        "The required field(s) %s in ObservationPostRequest is not found in the empty JSON string",
                        ObservationPostRequest.openapiRequiredFields.toString()
                ));
            }
        }
        ...
        for (Map.Entry<String, JsonElement> entry : entries) {
            if (!ObservationPostRequest.openapiFields.contains(entry.getKey())) {
                throw new IllegalArgumentException(String.format(
                        java.util.Locale.ROOT,
                        "The field `%s` in the JSON string is not defined in the `ObservationPostRequest` properties. JSON: %s",
                        entry.getKey(),
        ...
        if (jsonObj.get("observation") != null && !jsonObj.get("observation")
                                                          .isJsonNull()) {
            ObservationPostRequestObservation.validateJsonElement(jsonObj.get("observation"));
        }
    ...
    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        ...
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!ObservationPostRequest.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'ObservationPostRequest' and its subtypes
            }
            final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
            final TypeAdapter<ObservationPostRequest> thisAdapter
                    = gson.getDelegateAdapter(this, TypeToken.get(ObservationPostRequest.class));

            return (TypeAdapter<T>) new TypeAdapter<ObservationPostRequest>() {
                @Override
                public void write(JsonWriter out, ObservationPostRequest value) throws IOException {
                ...
                @Override
                public ObservationPostRequest read(JsonReader in) throws IOException {
