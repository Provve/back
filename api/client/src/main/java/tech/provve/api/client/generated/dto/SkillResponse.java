...
public class SkillResponse {
    ...
    static {
        ...
        openapiFields = new HashSet<String>(Arrays.asList("name", "vote_id"));
     ...
     * @throws IOException if the JSON Element is invalid with respect to SkillResponse
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        if (jsonElement == null) {
            if (!SkillResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
                throw new IllegalArgumentException(String.format(
                        java.util.Locale.ROOT,
                        "The required field(s) %s in SkillResponse is not found in the empty JSON string",
                        SkillResponse.openapiRequiredFields.toString()
                ));
            }
        }
        ...
        for (Map.Entry<String, JsonElement> entry : entries) {
            if (!SkillResponse.openapiFields.contains(entry.getKey())) {
                throw new IllegalArgumentException(String.format(
                        java.util.Locale.ROOT,
                        "The field `%s` in the JSON string is not defined in the `SkillResponse` properties. JSON: %s",
                        entry.getKey(),
        ...
        for (String requiredField : SkillResponse.openapiRequiredFields) {
        ...
        if (!jsonObj.get("vote_id")
                    .isJsonPrimitive()) {
            throw new IllegalArgumentException(String.format(
                    java.util.Locale.ROOT,
                    "Expected the field `vote_id` to be a primitive type in the JSON string but got `%s`",
                    jsonObj.get("vote_id")
                           .toString()
            ));
        }
    ...
    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        ...
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!SkillResponse.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'SkillResponse' and its subtypes
            }
            final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
            final TypeAdapter<SkillResponse> thisAdapter
                    = gson.getDelegateAdapter(this, TypeToken.get(SkillResponse.class));

            return (TypeAdapter<T>) new TypeAdapter<SkillResponse>() {
                @Override
                public void write(JsonWriter out, SkillResponse value) throws IOException {
                ...
                @Override
                public SkillResponse read(JsonReader in) throws IOException {
